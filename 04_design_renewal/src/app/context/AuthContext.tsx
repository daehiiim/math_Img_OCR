...
import {
  createLocalUiMockProfile,
  isLocalUiMockEnabled,
  LOCAL_UI_MOCK_AUTH_ERROR_MESSAGE,
  readLocalUiMockProfile,
} from "../lib/localUiMock";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncRemoteProfile = useCallback(async (fallbackProfile?: User | null) => {
    if (!browserSupabase || isLocalUiMock) {
      return;
    }
  ...
  useEffect(() => {
    if (isLocalUiMock) {
      setUser(readLocalUiMockProfile());
    ...
    if (!browserSupabase) {
      setAuthErrorMessage(LOCAL_UI_MOCK_AUTH_ERROR_MESSAGE);
      setIsLoading(false);
      return;
    }
  ...
  }, [isLocalUiMock, syncRemoteProfile]);
  ...
  const loginWithGoogle = useCallback(async () => {
    if (isLocalUiMock) {
      const profile = createLocalUiMockProfile();
      saveStoredProfile(profile);
      setUser(profile);
      setAuthErrorMessage(null);
      return profile;
  ...
  const value = useMemo(
    () => ({
      ...
      isLoading,
      isLocalUiMock,
      isSupabaseEnabled: hasSupabaseAuth && !isLocalUiMock,
      authErrorMessage,
      prepareLogin,
      loginWithGoogle,
      connectOpenAi,
