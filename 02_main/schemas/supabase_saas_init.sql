...
create table if not exists public.ocr_jobs (
  ...
  auto_detect_charged boolean not null default false,
  ...
  auto_detect_charged_at timestamptz,
  last_error text,
...
create table if not exists public.ocr_job_regions (
  ...
  selection_mode text not null default 'manual' check (selection_mode in ('manual', 'auto_full', 'auto_detected')),
  input_device text check (input_device in ('mouse', 'touch', 'pen', 'system')),
  warning_level text not null default 'normal' check (warning_level in ('normal', 'high_risk')),
  auto_detect_confidence double precision,
  ...
  markdown_version text,
  raw_transcript text,
  ordered_segments jsonb,
  question_type text check (question_type in ('multiple_choice', 'free_response')),
  parsed_choices jsonb,
  resolved_answer_index integer,
  resolved_answer_value text,
  answer_confidence double precision,
  verification_status text check (verification_status in ('verified', 'warning', 'unverified')),
