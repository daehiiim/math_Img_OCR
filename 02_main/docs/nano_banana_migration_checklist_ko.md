...
- `2026-03-19_region_action_credit_flags.sql` 파일이 배포 대상 브랜치에 포함되어 있는지 확인한다.
...
2. staging Supabase에 `2026-03-19_region_action_credit_flags.sql`을 이어서 적용한다.
3. `public.ocr_jobs`에 `ocr_charged`, `image_charged`, `explanation_charged` 컬럼이 생성됐는지 확인한다.
4. `public.ocr_job_regions`에 `ocr_charged`, `image_charged`, `explanation_charged`, `image_crop_path`, `styled_image_path`, `styled_image_model` 컬럼이 생성됐는지 확인한다.
5. `public.credit_ledger`의 `credit_ledger_reason_check` 제약에 `ocr_charge`, `image_stylize_charge`, `explanation_charge`가 포함됐는지 확인한다.
6. 기존 `was_charged=true` 행과 기존 산출물 데이터가 region/job 액션 플래그로 백필됐는지 확인한다.
7. staging backend 환경변수에 Nano Banana 4개 값을 넣고 재배포한다.
...
1. 저트래픽 시간에 production Supabase에 `2026-03-19_nano_banana_action_billing_upgrade.sql`과 `2026-03-19_region_action_credit_flags.sql`을 순서대로 적용한다.
2. backend 환경변수 4개가 production revision에 반영됐는지 확인한다.
3. backend를 재배포한 뒤 `GET /jobs/{id}` 응답에 `styled_image_url`, `styled_image_model`, region 액션 플래그가 내려오는지 확인한다.
