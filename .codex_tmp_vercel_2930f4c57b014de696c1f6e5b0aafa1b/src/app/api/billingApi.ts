...
import {
  createLocalUiMockCheckout,
  isLocalUiMockEnabled,
  LOCAL_UI_MOCK_BILLING_CATALOG,
  readLocalUiMockCheckoutStatus,
...
export async function getBillingCatalogApi(): Promise<BillingPlanResponse[]> {
  if (isLocalUiMockEnabled()) {
    return LOCAL_UI_MOCK_BILLING_CATALOG.map((plan) => ({ ...plan }));
  }
...
export async function createCheckoutSessionApi(payload: {
  ...
  if (isLocalUiMockEnabled()) {
    return createLocalUiMockCheckout(payload);
  }
...
export async function getCheckoutSessionStatusApi(checkoutId: string) {
  if (isLocalUiMockEnabled()) {
    return readLocalUiMockCheckoutStatus(checkoutId);
  }
