...
export function NewJobPage() {
  ...
  const handleDemoSelect = async (demo: (typeof DEMO_IMAGES)[0]) => {
    const response = await fetch(demo.url);
    const blob = await response.blob();
    const file = new File([blob], demo.name, { type: blob.type || "image/png" });
    setPreview({ url: demo.url, name: demo.name, width: demo.width, height: demo.height, file });
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {DEMO_IMAGES.map((demo) => (
                  <button
                    key={demo.name}
                    onClick={() => void handleDemoSelect(demo)}
