...
import { FileText, Image, Code, CheckCircle2, Loader2, Clock, Copy, Check, Pencil } from "lucide-react";
...
export function ResultsViewer({ regions, onSaveEditedSvg, onLoadRegionSvg }: ResultsViewerProps) {
  ...
  const [editingSvgText, setEditingSvgText] = useState<string>("");
        ...
        return (
          <Card key={region.id}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-[14px] flex items-center gap-2">
                  ...
                  {region.id}
                  <Badge variant="outline" className="text-[10px]">{region.type}</Badge>
                  {region.editedSvgUrl && <Badge className="text-[10px]">SVG 수정본</Badge>}
                </CardTitle>
                <Badge variant={region.status === "completed" ? "secondary" : "outline"} className="gap-1 px-[8px] py-[2px]">
            ...
            {region.status === "failed" && (
              <CardContent>
                <p className="text-[12px] text-destructive">영역 처리 실패: {region.errorReason || "원인 미상"}</p>
              </CardContent>
            )}
            {region.status === "completed" && (
              <CardContent>
                <Tabs defaultValue="ocr">
                  <TabsList>
                    <TabsTrigger value="ocr" className="gap-1.5"><FileText className="w-3.5 h-3.5" />OCR 결과</TabsTrigger>
                    <TabsTrigger value="svg" className="gap-1.5"><Image className="w-3.5 h-3.5" />SVG 벡터</TabsTrigger>
                    <TabsTrigger value="explain" className="gap-1.5"><FileText className="w-3.5 h-3.5" />해설</TabsTrigger>
                    <TabsTrigger value="mathml" className="gap-1.5"><Code className="w-3.5 h-3.5" />MathML</TabsTrigger>
                    <TabsTrigger value="raw" className="gap-1.5"><Code className="w-3.5 h-3.5" />Raw</TabsTrigger>
                  </TabsList>

                  <TabsContent value="ocr" className="mt-3">
                    <div className="bg-muted/50 rounded-lg p-4 relative">
                      <pre className="text-[13px] whitespace-pre-wrap font-mono">{region.ocrText}</pre>
                      <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={() => copyText(region.ocrText || "", `ocr-${region.id}`)}>
                        {copiedId === `ocr-${region.id}` ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                      </Button>
                    </div>
                    <p className="text-[11px] text-muted-foreground mt-2">ⓘ 백엔드 OCR 결과를 그대로 표시합니다.</p>
                  </TabsContent>
                  ...
                  <TabsContent value="svg" className="mt-3 space-y-3">
                    ...
                    {!isEditing ? (
                      ...
                      >
                        {svgLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Pencil className="w-4 h-4" />}
                        {svgLoading ? "SVG 불러오는 중..." : "SVG 도형 편집"}
                      </Button>
                    ) : (
                      <SvgEditorErrorBoundary
                        key={editingRegionId}
                        onError={(msg) => { setSvgEditorError(msg); setEditingRegionId(null); setEditingSvgText(""); }}
                      >
                    ...
                    <p className="text-[11px] text-muted-foreground">ⓘ SVG 편집기로 수정 후 저장하면 HWPX 내보내기에 반영됩니다.</p>
                  </TabsContent>

                  <TabsContent value="explain" className="mt-3">
                    <div className="bg-muted/50 rounded-lg p-4">
                      <pre className="text-[13px] whitespace-pre-wrap font-mono">{region.explanation || "해설 없음"}</pre>
                    </div>
                  </TabsContent>

                  <TabsContent value="mathml" className="mt-3">
                    <div className="bg-muted/50 rounded-lg p-4 relative overflow-x-auto">
                      <pre className="text-[11px] font-mono text-muted-foreground whitespace-pre-wrap">{region.mathml || "MathML 없음"}</pre>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="raw" className="mt-3">
                    <div className="bg-muted/50 rounded-lg p-4 relative overflow-x-auto">
                      ...
                      <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={() => copyText(JSON.stringify(region, null, 2), `raw-${region.id}`)}>
                        {copiedId === `raw-${region.id}` ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                      </Button>
                    </div>
                  </TabsContent>
                </Tabs>
