...
export function JobDetailPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const message = error instanceof Error ? error.message : "작업 로딩에 실패했습니다.";
  ...
  }, [jobId, job, isHydratingJob, hasHydrationError, hydrateJob]);
  ...
  const handleSaveRegions = useCallback(
    async (regions: Region[]) => {
      if (jobId) await saveRegions(jobId, regions);
    },
    [jobId, saveRegions]
  );
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              {job.status === "failed" && (
                <div className="text-center py-4">
                  <p className="text-[13px] text-destructive">파이프라인 실패</p>
