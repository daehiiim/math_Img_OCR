...
> = {
  text: { label: "텍스트", color: "#3b82f6", icon: MousePointer2 },
  diagram: { label: "도형", color: "#8b5cf6", icon: MousePointer2 },
  mixed: { label: "선택", color: "#f59e0b", icon: MousePointer2 },
};

function getRegionTypeConfig(type: string | undefined) {
  if (type === "text" || type === "diagram" || type === "mixed") {
    return regionTypeConfig[type];
  }
  return regionTypeConfig.mixed;
}
...
export function RegionEditor({
  ...
  const [pendingSelection, setPendingSelection] = useState<{ x1: number; y1: number; x2: number; y2: number } | null>(null);
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      // If there's a pending selection, check if click is inside it
      if (pendingSelection) {
        const inside =
          pos.x >= pendingSelection.x1 &&
          pos.x <= pendingSelection.x2 &&
          pos.y >= pendingSelection.y1 &&
          pos.y <= pendingSelection.y2;
        if (inside) {
          // Click inside pending selection — keep it active, don't start new draw
          return;
        }
        // Click outside — dismiss pending selection
    ...
    [tool, disabled, getScaledPos, pendingSelection]
  );
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setStartPoint(null);
    setCurrentPoint(null);
    // Keep selection box visible as pending
    setPendingSelection({ x1, y1, x2, y2 });
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => { setRegions([]); setPendingSelection(null); }}
      ...
      >
        ...
        {regions.map((region) => {
          ...
          const cfg = getRegionTypeConfig(region.type);
        ...
        {/* Drawing preview (while actively dragging) */}
        ...
        {/* Persistent pending selection (stays until click outside) */}
        {!drawing && pendingSelection && (
          <div
            className="absolute border-2 border-dashed rounded-sm pointer-events-none"
            style={{
              left: toPercent(pendingSelection.x1, imageWidth),
              top: toPercent(pendingSelection.y1, imageHeight),
              width: toPercent(
                pendingSelection.x2 - pendingSelection.x1,
                imageWidth
              ),
              height: toPercent(
                pendingSelection.y2 - pendingSelection.y1,
                imageHeight
              ),
              borderColor: regionTypeConfig[selectedType].color,
              backgroundColor: `${regionTypeConfig[selectedType].color}20`,
      ...
      {!disabled && (
        <>
          <div className="flex justify-end">
            <Button onClick={() => void handleSave()} disabled={regions.length === 0 || isSaving} className="gap-2">
              <Plus className="w-4 h-4" />
              {isSaving ? "저장 중..." : `영역 저장 (${regions.length}개)`}
            </Button>
          </div>
          {saveError && <p className="text-[12px] text-destructive mt-2">{saveError}</p>}
        </>
      )}
    </div>
  );
